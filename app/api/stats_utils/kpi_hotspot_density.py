...
async def get_hotspot_density(
    ...
    high_confidence_query = select(func.count(Media.id)).where(
    ...
    try:
        ...
        snapped = ST_SnapToGrid(Media.location, HOTSPOT_GRID_RESOLUTION_DEGREES)

        # Count distinct grid cells (each represents a unique hotspot area)
        cluster_query = select(func.count(func.distinct(snapped))).where(
            Media.uploader_id == user_id,
            Media.confidence >= HOTSPOT_CONFIDENCE_THRESHOLD,
