...
from app.api.upload_utils.hf_upload import upload_video_frames_to_hf
...
async def process_video_media(
    ...
    async with AsyncSessionLocal() as session:
        media_result = await session.execute(select(Media).where(Media.id == media_id))
        ...
        try:
            await asyncio.to_thread(extract_srt_from_video, local_video_path, srt_path)
        except MissingTelemetryError as e:
            media.status = MediaStatus.FAILED
            media.failed_reason = str("No embedded GPS telemetry track found in video.")
            session.add(
                create_status_change_log(media.id, MediaStatus.FAILED, detail=str(e))
            )
        ...
        while current_frame_idx < total_frames:
            ...
            frame_lat, frame_lng = get_location_at_timestamp(srt_path, timestamp_sec)
            ...
            if frame_lat is not None and frame_lng is not None:
                ...
                for det in detections:
                    ...
                    hf_file_path = f"media/{user_id}/{date_str}/{media_id}_{filename}"
    ...
    # cleanup temp files
    for local_path, _ in frames_to_upload:
        try:
    ...
    try:
        if await asyncio.to_thread(os.path.exists, srt_path):
