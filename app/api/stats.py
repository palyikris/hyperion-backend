...
async def get_mean_time_to_process(
    ...
    Stage 1 - Subquery:
    - For each (media_id, worker_name) combination in MediaLog:
      - MIN(timestamp) = when processing started
      - MAX(timestamp) = when processing completed
    - Only includes READY media (successful completions)
    
    Stage 2 - Aggregation:
    - Calculates AVG(end_time - start_time) per worker
    - Uses EXTRACT('epoch', ...) to convert interval to seconds
    - Counts tasks per worker for context
    ...
    """
    # Stage 1: Subquery to compute start/end times per media per worker
    # This gives us the processing duration for each task
    subquery = (
        select(
            MediaLog.media_id,
    ...
    worker_query = (
        select(
            subquery.c.worker_name,
            func.avg(
                func.extract('epoch', subquery.c.end_time - subquery.c.start_time)
            ).label("avg_seconds"),
            func.count(subquery.c.media_id).label("task_count")
        )
        .where(subquery.c.worker_name.isnot(None))  # Exclude unassigned tasks
        .group_by(subquery.c.worker_name)
    )
    ...
    # Stage 2b: Calculate overall average (not grouped by worker)
    overall_query = (
        select(
            func.avg(
                func.extract('epoch', subquery.c.end_time - subquery.c.start_time)
